  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data.error || 'Failed to reset password. The token may be expired.');
      }
    } catch {
    ...
    <>
      ...
      {!message && (
        <form className="space-y-stack-md" onSubmit={handleSubmit}>
          ...
          {/* Confirm Password */}
          <div className="space-y-1">
            <label className="font-label-sm text-label-sm text-on-surface-variant block ml-1" htmlFor="confirmPassword">
              CONFIRM PASSWORD
            </label>
            <div className="relative">
              <input
                className="w-full px-4 py-3 rounded-xl border border-outline-variant bg-surface-bright font-body-md text-on-surface input-academic transition-all"
                id="confirmPassword" type={showPw ? 'text' : 'password'} required
                placeholder="••••••••"
          ...
          {/* Submit */}
          <button
            className="w-full bg-primary text-on-primary font-headline-md text-headline-md py-4 rounded-xl hover:opacity-90 active:scale-[0.98] transition-all flex items-center justify-center space-x-2 shadow-lg shadow-primary/5 mt-4"
            type="submit"
      ...
      <main className="flex-grow flex items-center justify-center pt-24 pb-stack-xl px-margin-x">
        <div className="w-full max-w-md bg-surface-container-lowest rounded-xl p-stack-lg border border-primary/5 transition-all duration-500">
          ...
          <div className="text-center mb-stack-lg">
            <h1 className="font-headline-lg text-headline-lg text-primary mb-2">Create Password</h1>
            <p className="font-body-sm text-body-sm text-on-surface-variant">Enter a new secure password</p>
          </div>

          <Suspense fallback={<div className="flex justify-center p-8"><div className="w-8 h-8 border-4 border-primary/20 border-t-primary rounded-full animate-spin"></div></div>}>
            <ResetPasswordForm />
          </Suspense>
